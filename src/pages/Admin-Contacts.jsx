...
const AdminContacts = () => {
  ...
  const getContactData = async () => {
    try {
      const response = await fetch("https://mernbackend-tex1.onrender.com/api/admin/contacts", {
        method: 'GET',
  ...
  const deleteContactById = async (id) => {
    try {
      const response = await fetch(`https://mernbackend-tex1.onrender.com/api/admin/contacts/delete/${id}`, {
        method: 'DELETE',
        headers: { Authorization: authorizationToken },
      });
      if (response.ok) {
