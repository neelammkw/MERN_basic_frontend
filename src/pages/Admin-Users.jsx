...
const AdminUsers = () => {
  ...
  const getAllUserData = async () => {
    try {
      const response = await fetch('https://mernbackend-tex1.onrender.com/api/admin/users', {
        method: 'GET',
  ...
  const deleteUser = async (id) => {
    try {
      const response = await fetch(`https://mernbackend-tex1.onrender.com/api/admin/users/delete/${id}`, {
        method: 'DELETE',
        headers: { Authorization: authorizationToken },
      });
      if (response.ok) {
